...
def main():
    ...
    config_backup_file = "e1_vlan_identification_output.json"
    master_backup = []

    print(f"\nStarting VLAN configuration backup for closet '{args.closet}'...")
    ...
    for device in devices:
        ...
        try:
            with EXOSHTTPSManager(device, username=ENV_USER, password=ENV_PASS) as connection:
                config_data = connection.get_vlans()
                ...
                print(f"    -- Retrieved VLAN configuration for {hostname} ({host_ip})")
